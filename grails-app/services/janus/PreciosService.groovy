...
//    def getPrecioFecha (fecha,items) {
//        def cn = dbConnectionService.getConnection()
//        def itemsId = items
//        def res =[]
//        def sql = "SELECT r1.item__id,i.itemcdgo,(SELECT r2.rbpc__id from rbpc r2 where r2.item__id=r1.item__id and r2.rbpcfcha = max(r1.rbpcfcha)) from rbpc r1,item i where r1.item__id in (${itemsId}) and r1.rbpcfcha < '${fecha.format('MM-dd-yyyy')}' and i.item__id=r1.item__id group by 1,2 order by 2"
//
//        println(sql)
//
//        cn.eachRow(sql.toString()) { row ->
