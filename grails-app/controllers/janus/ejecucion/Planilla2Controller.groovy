...
//        println "planilla "+planilla+"  "+fechaOferta+"  "+fechaAnticipo+" pers "+periodos+"  "+perOferta+"  "+perAnticipo
        ...
        cs.each {c->
            ...
            tablaBo+="<th class='number'>"+c.valor+"</th>"
            totalCoef+=c.valor
            periodosAnticipo.each {p->
                ...
                tablaBo+="<td class='number'>"+valor+"</td>"
        ...
        tablaBo+="</tbody><tfoot>"
        tablaBo+="<tr>"+"<th>TOTALES</th><th class='number'>${totalCoef}</th>"



        periodosAnticipo.each {p->
            tablaBo+="<td></td><th class='number'>${p.totalLiq}</th>"
        }
        periodosAvances.each {p->
            tablaBo+="<td></td><th class='number'>${p.totalLiq}</th>"
        }
        ...
        (periodosAnticipo+periodosAvances).each {p->
            if(p.titulo!="OFERTA"){
                ...
                if(p.titulo=="ANTICIPO"){
                    ...
                    tablaP0 += "<td></td>"
                    tablaP0 += "<td></td>"
                    tablaP0 += "<td class='number'>${p.p0}</td>"
                }else{
                    def dias = p.fechaFin-p.fechaIncio+1
                    diasTot+=dias
                    tablaP0 += "<th>${fechaConFormato(p.fechaIncio,'MMM-yy')}</th>"
                    tablaP0 += "<th>(${dias})</th>"
                    tablaP0 += "<td class='number'>${p.parcialCronograma}</td>"
                    act+= p.parcialCronograma
                    totCrono+=p.parcialCronograma
                    tablaP0 += "<td class='number'>${act}</td>"
                    tablaP0 += "<td class='number'>${p.parcialPlanilla}</td>"
                    act2+= p.parcialPlanilla
                    totPlan+=p.parcialPlanilla
                    tablaP0 += "<td class='number'>${act2}</td>"
                    tablaP0 += "<td class='number'>${p.p0}</td>"
...
//                    bodyMultaRetraso += "</tr>"
        ...
        tablaP0 += "<th class='number'>${totCrono}</th>"
        tablaP0 += "<td class='number'></td>"
        tablaP0 += "<th class='number'>${totPlan}</th>"
        tablaP0 += "<td class='number'></td>"
        tablaP0 += '</tfoot>'
        tablaP0 += '</table>'
...
//                valor = (valor*c.valor).round(3)
        ...
        tablaBo+="<tr>"+"<th>TOTALES</th><th class='number'>${totalCoef}</th>"
        periodos.each {p->
            tablaBo+="<td></td><th class='number'>${p.total}</th>"
        }
        periodos2.each {p->
            tablaBo+="<td></td><th class='number'>${p.total}</th>"
        }
...
//                println "parcial crono += "+((pc/diasPr)*dias2).round(2)+"   = "+parcialCronograma+"     ("+pc+")   "+diasPr
        ...
        (periodos+periodos2).eachWithIndex { p, i->
            if(i==0) { //oferta
                tr1+="<th>"+p.titulo+"</th>"
                tr2+="<th>"+fechaConFormato(p.fechaIncio,"MMM-yy")+"</th>"
                tr3+="<th class='number'>"+p.planilla.contrato.porcentajeAnticipo+"%</th>"
            } //oferta
            else if(i==1) { //anticipo
                tr1+=tdRowSpan
                tr2+="<th>"+p.titulo+"<br/>"+fechaConFormato(p.fechaIncio, "MMM-yy")+"</th>"
            ...
            periodos.eachWithIndex { per, j ->
                if(j == 0) { // es la oferta
                    ...
                    if(i == 0) { //es mano de obra
                        ...
                        tablaFr+="<td class='number'><div>${p.valor}</div><div class='bold'>${per.total}</div></td>"
                        valor=per.total
                    } else{
...
//                    println "error "+p.indice+" "+p.indice.id+"  "+per.periodo.id+"   "+vlin+"  "+vlinOferta+"  "+p.valor+"  "+"  "+per.periodo.id
        ...
        tr7+="<th colspan='2'>REAJUSTE A PLANILLAR</th><th colspan='${periodos.size()+periodos2.size()-1}' class='number'>"+((reajusteTotal-reajusteAnterior).toDouble().round(2))+"</th>"
        ...
        tablaMl += "<th class='number'>${totalMultaRetraso}</th>"
        tablaMl += '</tr>'
        tablaMl += '</tfoot>'
        tablaMl += '</table>'
...
//            totalMulta = (totalContrato) * (prmlMulta / 1000) * retraso
        ...
        pMl += "</tr>"
        pMl += "<tr>"
        pMl += '<th class="tal">Multa</th> <td>' +  prmlMulta + "&#8240; de \$" +  totalContrato + "</td>"
        pMl += "</tr>"
        pMl += "<tr>"
        pMl += '<th class="tal">Total multa</th> <td>$' +  totalMulta + "</td>"
        pMl += "</tr>"
        pMl += '</table>'
        ///////////////////////////////////////////************************************fin**********************////////////////////////////
        ...
        periodos.each {p->
            if(p.titulo!="OFERTA"){
                ...
                if(p.titulo=="ANTICIPO"){
                    ...
                    tablaP0 += "<td></td>"
                    tablaP0 += "<td></td>"
                    tablaP0 += "<td class='number'>${planilla.valor}</td>"
                    if(p.p0==0 || override){
                        p.p0=planilla.valor
                        if(!p.save(flush: true)){
                            println "error p0 "+p
